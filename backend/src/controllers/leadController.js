...
const inferFlaw = (website, ratingStr) => {
...
const startScrapingTask = async (req, res) => {
    ...
    try {
        ...
        const url = `https://www.google.com/maps/search/${encodeURIComponent(searchKeyword)}+in+${encodeURIComponent(searchLocation)}`;
        ...
        const scraped = await page.evaluate(() => {
            const places = Array.from(document.querySelectorAll('a[href*="/maps/place/"]')).map((a) => a.closest('div')?.parentElement?.parentElement || a);
            const seen = new Set();
            return places
                .map((el) => {
                ...
                const websiteEl = Array.from(el.querySelectorAll('a[href^="http"]')).find((a) => !a.href.includes('google.com') && !a.href.includes('/maps/'));
        ...
        const leads = scraped.map((raw, index) => {
...
exports.startScrapingTask = startScrapingTask;
