...
router.get('/conversations', async (req, res) => {
    try {
        ...
        let userIdQuery;
        try {
            userIdQuery = new mongoose_1.Types.ObjectId(String(userId));
        }
        catch {
        ...
        const result = await Promise.all(conversations.map(async (conv) => {
            try {
                ...
                const lastMsg = await Message_1.default.findOne({ conversation: conv._id })
                    .sort({ createdAt: -1 })
                    .lean();
                const unread = await Message_1.default.countDocuments({
...
router.get('/find', async (req, res) => {
    try {
        ...
        let conversation = await Conversation_1.default.findOne({ participantA: a, participantB: b });
        if (!conversation) {
...
router.get('/:conversationId/messages', async (req, res) => {
    try {
        ...
        try {
            conversationIdQuery = new mongoose_1.Types.ObjectId(String(conversationId));
        }
        catch {
            // If conversationId is not a valid ObjectId, use it as a string
            conversationIdQuery = String(conversationId);
        }
        const messages = await Message_1.default.find({ conversation: conversationIdQuery })
            .sort({ createdAt: 1 })
...
router.post('/seed', async (_req, res) => {
    try {
        ...
        for (const u of demoUsers) {
            let user = await User_1.default.findOne({ email: u.email });
            if (!user) {
        ...
        let conv = await Conversation_1.default.findOne({ participantA: pa, participantB: pb });
        if (!conv) {
        ...
        for (let i = 0; i < sampleMessages.length; i++) {
            const exists = await Message_1.default.exists({
                conversation: conv._id,
